...
```text
[Start] --> [Write Dockerfile] --> [Create .dockerignore] --> [Build Docker Image]
                                                                     |
                                                                     v
      [End] <-- [Access Application] <-- [Run Docker Container]
```
...
```text
[Copy package*.json] --> [Run npm install] --> [Copy rest of the code] --> [Build Image]
```
...
```text
[FROM node:18-alpine] --> [COPY package*.json ./]
                                   |
                                   v
[COPY . .] <-- [RUN npm install --production]
    |
    v
...
[Copy package.json & package-lock.json] --> [Run npm install]
                                                   |
                                                   v
[Build Image with Cached Dependencies] <-- [Copy rest of the code]
```
